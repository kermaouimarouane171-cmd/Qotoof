...
import { enforceRateLimit, checkReviewCreateRate, checkReviewReplyRate } from '@/utils/rateLimiter'
...
const reviewService = {
  async createReview({ orderId = null, productId = null, vendorId, userId, rating, comment = '' }) {
    ...
    if (!orderId) {
      throw new Error('معرّف الطلب مطلوب لإنشاء تقييم.')
    }

    // Rate limit: 5 reviews per 10 minutes per user
    enforceRateLimit(checkReviewCreateRate, userId)
    ...
    if (comment && String(comment).length > MAX_COMMENT_LENGTH) {
      throw new Error(`التعليق طويل جداً. الحد الأقصى ${MAX_COMMENT_LENGTH} حرفاً.`)
    }
    ...
    const payload = {
      ...
      comment: comment ? sanitizeText(comment, { maxLength: 1000, allowNewlines: true, collapseWhitespace: false }) : null,
    }
  ...
  async replyToReview({ reviewId, vendorId, replyText }) {
    ...
    // Rate limit: 10 replies per 10 minutes per vendor
    enforceRateLimit(checkReviewReplyRate, vendorId)

    // Validate reply length (max 1000 characters)
    const MAX_REPLY_LENGTH = 1000
    if (replyText && String(replyText).length > MAX_REPLY_LENGTH) {
      throw new Error(`الرد طويل جداً. الحد الأقصى ${MAX_REPLY_LENGTH} حرفاً.`)
    }

    const trimmedReply = sanitizeText(String(replyText || ''), { maxLength: MAX_REPLY_LENGTH, allowNewlines: true, collapseWhitespace: false }).trim()
