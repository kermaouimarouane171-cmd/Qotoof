...
import { normalizeQuantity } from '../domain/cartQuantity'
  ...
  try {
    ...
    const translations = {
      en: {
        ...
        'cart.multiVendorWarning': 'You already have items from another vendor in your cart. Checkout supports one vendor at a time.',
      },
      fr: {
        ...
        'cart.multiVendorWarning': 'Vous avez déjà des articles d\'un autre vendeur dans votre panier. Le paiement ne prend en charge qu\'un seul vendeur à la fois.',
      },
      ar: {
        ...
        'cart.multiVendorWarning': 'لديك منتجات من بائع آخر في السلة. إتمام الطلب يكون لبائع واحد فقط في كل مرة.',
      }
    }
  ...
  persist(
    (set, get) => ({
      ...
      addItem: (product, quantity) => {
        ...
        // Warn when adding items from multiple vendors (checkout is single-vendor)
        const existingVendorId = items.find(item => item.id !== product.id)?.vendor_id
        if (existingVendorId && existingVendorId !== product.vendor_id) {
          toast(
            getTranslation('cart.multiVendorWarning', 'You already have items from another vendor in your cart. Checkout supports one vendor at a time.'),
      ...
      /**
       * Add a negotiated item to the cart with a locked price.
       * The price will not change even if the product price updates.
       * If the same product already exists in the cart (non-negotiated),
       * it is replaced with the negotiated version.
       */
      addNegotiatedItem: (negotiation) => {
        const { items } = get()
        const product = negotiation.product || {}
        const lockedPrice = Number(negotiation.proposed_price)
        const quantity = Number(negotiation.proposed_quantity) || 1
        ...
        const negotiatedItem = {
          ...
          vendor_name: product.vendor?.store_name || product.vendor_name || product.store_name || '',
          image_url: product.image_url || product.images?.[0]?.url || product.image || null,
          category: product.category || null,
          subcategory: product.subcategory || null,
        }

        const existingIdx = items.findIndex(item => item.id === negotiatedItem.id)
        let nextItems
        if (existingIdx >= 0) {
          nextItems = items.map((item, idx) => idx === existingIdx ? negotiatedItem : item)
        } else {
          nextItems = [...items, negotiatedItem]
        }
      ...
      validateCart: async () => {
        ...
        try {
          ...
          for (const item of items) {
            ...
            // Skip price change check for negotiated items (locked price)
            ...
            // For negotiated items, preserve locked_price and don't update price_per_unit
            const newPrice = fresh.price_per_unit
            if (item.is_negotiated) {
              validItems.push({
                ...item,
